...
func (p *Parser) ParseWithClaims(tokenString string, claims Claims, keyFunc Keyfunc) (*Token, error) {
	...
	if p.validMethods != nil {
		...
		if !signingMethodValid {
			// the signing method is not in the listed set
...
func (p *Parser) ParseUnverified(tokenString string, claims Claims) (token *Token, parts []string, err error) {
	...
	if headerBytes, err = codec.Base64.DecodeUrlSafeStr(parts[0]); err != nil {
	...
	if claimBytes, err = codec.Base64.DecodeUrlSafeStr(parts[1]); err != nil {
		return token, parts, &ValidationError{Inner: err, Errors: ValidationErrorMalformed}
	}
	...
	// JSON Decode.  Special case for a map type to avoid weird pointer behavior
